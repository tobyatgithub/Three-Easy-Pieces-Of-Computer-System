...
class malloc:
    ...
    def malloc(self, size):
        ...
        for i in range(len(self.freelist)):
            ...
            count += 1
            if esize >= size and (
                (self.policy == "BEST" and esize < bestSize)
                or (self.policy == "WORST" and esize > bestSize)
                or (self.policy == "FIRST")
            ):
                bestAddr = eaddr
                bestSize = esize
                bestIdx = i
...
m = malloc(
    int(options.heapSize),
    int(options.baseAddr),
    int(options.headerSize),
    options.policy,
    options.order,
    options.coalesce,
    options.alignment,
    ...
    while j < int(options.opsNum):
        ...
        if random.random() < percent:
            ...
            if options.solve == True:
                print(
                    " returned %d (searched %d elements)"
